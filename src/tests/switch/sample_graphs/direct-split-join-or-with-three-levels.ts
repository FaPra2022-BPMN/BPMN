...
export class DirectSplitJoinOr extends TestGraph {
    ...
    create(): BpmnGraph {
        ...
        this.graph.addEdge(new BpmnEdge(this.edge_idx, startEvent, gatewaySplitOr));

        //SPLIT_OR --> task1 Flug suchen
        let task1 = this.createNode(new BpmnTaskManual("Task1"), "Flug suchen");
        this.graph.addEdge(new BpmnEdge(this.edge_idx, gatewaySplitOr, task1));

        //task1 Flug suchen --> task3 Flug buchen
        let task3 = this.createNode(new BpmnTaskSending("Task3"), "Flug buchen");
        this.graph.addEdge(new BpmnEdge(this.edge_idx, task1, task3));
        ...
        this.graph.addEdge(new BpmnEdge(this.edge_idx, task4, eventIntermed2));

        //eventIntermed2 Buchungsbestätigung --> JOIN_OR
        this.graph.addEdge(new BpmnEdge(this.edge_idx, eventIntermed2, gatewayJoinOr));

        //SPLIT_OR --> JOIN OR
        this.graph.addEdge(new BpmnEdge(this.edge_idx, gatewaySplitOr, gatewayJoinOr));

        //JOIN_OR --> task5 Reiseunterlagen
        let task5 = this.createNode(new BpmnTaskService("Task5"), "Reiseunterlagen speichern");
        this.graph.addEdge(new BpmnEdge(this.edge_idx, gatewayJoinOr, task5));
